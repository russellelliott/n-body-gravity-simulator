...
export const TopHUD: React.FC<TopHUDProps> = ({
  ...
  return (
    <div className="absolute top-4 left-4 z-30 flex flex-wrap items-center gap-3 select-none pointer-events-none">
      ...
      <div className="pointer-events-auto hidden lg:flex items-center gap-1.5 bg-slate-950/70 backdrop-blur-xl border border-slate-800/80 px-2 py-1 rounded-2xl text-[10px] font-semibold text-slate-400 shadow-xl">
        <span className="px-1 text-slate-500 uppercase tracking-wider text-[9px]">Scenarios:</span>
        {[
          { id: 'solar_system', label: 'Solar System' },
          { id: 'binary_stars', label: 'Binary Stars' },
          { id: 'galaxy_disk', label: 'Galaxy Disk' },
          { id: 'planet_smash', label: 'Planet Smash' }
        ].map((p) => (
          <button
            key={p.id}
            onClick={() => onQuickLoadPreset(p.id)}
            className="px-2 py-1 bg-slate-900/80 hover:bg-slate-800 hover:text-white border border-slate-800/80 rounded-xl transition-colors"
          >
            {p.label}
          </button>
        ))}
